...
//fechar o modal qaundo clicar fora
...
checkoutBtn.addEventListener("click", function () {
    ...
    if (!isOpen) {
        Toastify({
            text: "Opah! o restaurante esta fechado!!!",
            duration: 3000,
            close: true,
            gravity: "top",
            position: "rigt",
    ...
    const cartItems = cart.map((item) => {
        return (
            ` ${item.name} Quantidade: (${item.quanti}) Preco: R$${item.price} |`
        )
    }).join("")

    const message = encodeURIComponent(cartItems)
    const phone = "27999940845"
...
//verificra a hora e manipular o card do horario
